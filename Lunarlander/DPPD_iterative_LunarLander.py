...
from model import StudentActorLunarlander, ActorLunarlander
from utils.logger_utils import get_logger
from argparse import ArgumentParser
from Lunarlander.evaluate_lunarlander import evaluate
...
def main():
    logger = get_logger(FLAGS.DPPD_dir + "/DPPD_ITERATIVE")
    logger.info(" ------------- START: -------------")
    ...
    teacher = ActorLunarlander(input_size=dense_config.input_size, output_size=dense_config.output_size,
                               model_path=FLAGS.teacher_path, scope='Teacher')
    ...
    prune_step_path = FLAGS.DPPD_dir + "/prune_step_"
    policy_step_path = FLAGS.DPPD_dir + "/policy_step_"
    ...
    compressed_agent = StudentActorLunarlander(input_size=student_config.input_size,
    ...
    while not converge:
        ...
        sparsity_vs_accuracy = iterative_pruning_policy_distilliation(logger=logger, agent=compressed_agent,
                                                                      ...
                                                                      accumulate_experience_fn=accumulate_experience_lunarlander,
                                                                      objective_score=student_config.OBJECTIVE_SCORE,
                                                                      lower_bound=student_config.LOWER_BOUND)
        plot_graph(data=sparsity_vs_accuracy, name=FLAGS.DPPD_dir + "/initial size {}%,  Pruning_step number {}"
        ...
        logger.info(
            "----- redundancy for each layer at step {} is {} -----".format(iteration, redundancy_at_each_layer))
    ...
    plot_graph(data=accuracy_vs_size, name=FLAGS.DPPD_dir + "/accuracy_vs_size", figure_num=iteration + 1,
               xaxis='NNZ params', yaxis='Accuracy')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--DPPD_dir',
        type=str,
        default=student_config.iterative_DPPD,
    ...
    parser.add_argument(
        ...
        type=int,
        default=student_config.n_epochs,
        help='number of epoches to do pruning')
    parser.add_argument(
        '--eval_epochs',
        type=int,
        default=100,
